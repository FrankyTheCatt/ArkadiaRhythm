...
public partial class Level : Node2D
{
	...
	if (positions.Count < 4)
	{
		...
		damageNoteTimer.Connect("timeout", new Callable(this, nameof(SpawnKeyObjectInLane)));
...
private void SpawnKeyObjectInLane(int key)
{
	...
	{
		int randomIndex = (int)GD.Randi() % 4; // Genera un índice aleatorio
		if (randomIndex < 0 || randomIndex >= positions.Count) return;
	...
	public void TakeDamage(float damageAmount)
	{
		...
		{	
			...
			// Lógica adicional si el jugador pierde toda la vida
		}
	}
